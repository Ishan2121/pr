...
// import cloudinary_client from '../config/cloudinary.js'
...
async function uploadFileToCLoudinary(tempFilePath, folder) {
...
router.post('/upload', async (req, res) => {
    try {
        ...
        const { originalname, mimetype, size, path } = req.files.asset
        const { brandName, description } = req.body
        const extension = req.files.asset.name.split(".");
        const fileType = mimetype.split('/')[0];
        let SchemaToUse = schemaMapping[fileType];

        const name = Date.now();
        req.files.asset.mv(`uploads/${name}.${extension[1]}`);
        req.files.asset.tempFilePath = `uploads/${name}.${extension[1]}`;
        ...
        const result = await uploadFileToCLoudinary(req.files.asset.tempFilePath, options)
        ...
        console.log(assetURL, " ", assetPublicId, " ", tags)
